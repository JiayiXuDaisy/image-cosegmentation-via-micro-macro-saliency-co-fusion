...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
  ...
  {
    printf("input 1 must be a single array\n");
    return;
  } // H
  if (mxGetClassID(prhs[1]) != mxSINGLE_CLASS)
  {
    printf("input 2 must be a single array\n");
    return;
  } // params
  if (mxGetClassID(prhs[2]) != mxSINGLE_CLASS)
  {
    printf("input 3 must be a single array\n");
    return;
  } // grid
  if (mxGetClassID(prhs[3]) != mxSINGLE_CLASS)
  {
    printf("input 4 must be a single array\n");
    return;
  } // ostable
  if (mxGetClassID(prhs[4]) != mxSINGLE_CLASS)
  {
    printf("input 5 must be a single array\n");
    return;
  } // y x
  if (nlhs != 1)
  {
  ...
  // coordinates
  float *pcoord = (float *)mxGetData(prhs[4]);
  float y = pcoord[0];
  float x = pcoord[1];
  int s = pcoord[2];
  ...
  memset(desc_out, 0, sizeof(float) * DS);
  compute_descriptor(H, params, grid, y, x, ostable[s], desc_out);
}
...
inline void compute_descriptor(const float *H, const float *params, const float *grid, float y, float x, float shift, float *desc_out)
{
  ...
  if (si == 0 && li == 0)
    u_compute_descriptor_00(H, params, grid, y, x, shift, desc_out);
  else if (si == 0 && li == 1)
    u_compute_descriptor_01(H, params, grid, y, x, shift, desc_out);
  else if (si == 1 && li == 0)
    u_compute_descriptor_10(H, params, grid, y, x, shift, desc_out);
  else if (si == 1 && li == 1)
    u_compute_descriptor_11(H, params, grid, y, x, shift, desc_out);

  if (nt == 0)
    return;
  else if (nt == 1)
    normalize_partial(desc_out, params[1], params[7]);
  else if (nt == 2)
    normalize_full(desc_out, params[0]);
  else if (nt == 3)
    normalize_sift(desc_out, params[0]);
  else
...
inline void u_compute_descriptor_10(const float *H, const float *params, const float *grid, float y, float x, float shift, float *desc_out)
{
  ...
  for (g = 0; g < hn; g++)
  {
    ...
    for (j = 0; j < hq - ishift; j++)
      hist[j] = (1 - a) * (1 - b) * cube[(j + ishift) * hw];
    for (cnt = 0; cnt < ishift; cnt++, j++)
      hist[j] = (1 - a) * (1 - b) * cube[cnt * hw];

    // B
    cube = H + (c - 1) * hw * hq + iy * w + ix + w;
    for (j = 0; j < hq - ishift; j++)
      hist[j] += b * (1 - a) * cube[(j + ishift) * hw];
    for (cnt = 0; cnt < ishift; cnt++, j++)
      hist[j] += b * (1 - a) * cube[cnt * hw];

    // C
    cube = H + (c - 1) * hw * hq + iy * w + ix + 1;
    for (j = 0; j < hq - ishift; j++)
      hist[j] += a * (1 - b) * cube[(j + ishift) * hw];
    for (cnt = 0; cnt < ishift; cnt++, j++)
      hist[j] += a * (1 - b) * cube[cnt * hw];

    // D
    cube = H + (c - 1) * hw * hq + iy * w + ix + w + 1;
    for (j = 0; j < hq - ishift; j++)
      hist[j] += a * b * cube[(j + ishift) * hw];
    for (cnt = 0; cnt < ishift; cnt++, j++)
      hist[j] += a * b * cube[cnt * hw];
  }
}
inline void u_compute_descriptor_11(const float *H, const float *params, const float *grid, float y, float x, float shift, float *desc_out)
{
