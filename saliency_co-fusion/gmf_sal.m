function [ output_args ] = gmf_sal(path,patho)
...
imgRoot=path;%'./data/women/';% test image path
saldir=patho;%'./data/women/saliency_maps/sal6/';% the output path of the saliency map
...
% assign the saliency value to each pixel
    ...
    imwrite(mat2gray((mat2gray(mapstage2)+mat2gray(mapstage1))/2),outname);
    [~,~,ext]=fileparts(imnames(ii).name);
    if ~strcmp(ext,'.bmp')
        delete(imname)
    end
