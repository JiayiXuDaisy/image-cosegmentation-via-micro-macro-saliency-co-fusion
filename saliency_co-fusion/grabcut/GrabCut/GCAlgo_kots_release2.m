function [CurrRes,mask] = GCAlgo_kots_release2( im, fixedBG,otsu,para2,im2)
im=double(im);
para2=1-para2;
fixedBG=1-fixedBG;
Beta =0.3;
...
gradH = (im2(:,2:end,:) - im2(:,1:end-1,:));%.*repmat(mat2gray(max(im2(:,2:end),im2(:,1:end-1))),[1 1 3]);
gradV = (im2(2:end,:,:) - im2(1:end-1,:,:));%.*repmat(mat2gray(max(im2(2:end,:),im2(1:end-1,:))),[1 1 3]);

gradH = sum(gradH.^2, 3);
gradV = sum(gradV.^2, 3);

gradH2 = im(:,2:end,:) - im(:,1:end-1,:);
gradV2 = im(2:end,:,:) - im(1:end-1,:,:);

gradH2 = sum(gradH2.^2, 3);
gradV2 = sum(gradV2.^2, 3);

gradH=(gradH+gradH2)/2;
gradV=(gradV+gradV2)/2;
...
for iter=1:maxIterations
   ...
   prevLabelo=1-prevLabel;
    b1=bgIds;
    f1=find(prevLabel<(para2/2));
    end
   ...
   %end
    ...
    fgLogPL=fgLogPL-0.5*log(max(eps,im2));
    bgLogPL=bgLogPL-0.5*log(1-min(1-eps,im2));
    
    %fgLogPL(f1)= fgLogPL(f1)-5*log(prevLabelo(f1));
    %bgLogPL(b1)= bgLogPL(b1)-5*log(1-prevLabelo(b1));
